...
export default function SeasonalRadialChart({ data, width, height }) {
  const MARGIN = {
    left: 16,
    right: 16,
    top: 16,
  ...
  const items = data.map((d) => String(d.name.en));
  ...
  return (
    <svg width={width} height={height}>
      <g transform={`translate(${centreX}, ${centreY})`}>
        ...
        {data.map((item, i) => {
          const innerRadius = thicknessScale(item.name.en) ?? 0;
